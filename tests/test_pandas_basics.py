...
# ==============================================================================
# ADVANCED FUNCTIONS TESTS (New AI-Enhanced Functions)
# ==============================================================================

class TestValidateCoordinateData:
    """Tests for the validate_coordinate_data function (Advanced Function 6)."""
    
    @pytest.fixture
    def sample_coordinate_data(self):
        """Create sample data with coordinate validation challenges."""
        return pd.DataFrame({
            'station_id': ['STN_001', 'STN_002', 'STN_003', 'STN_004', 'STN_005', 'STN_006'],
            'latitude': [33.4484, 91.0000, 33.4484, -200.0, np.nan, 45.5234567890123456],
            'longitude': [-112.0740, -112.0740, -112.0740, -112.0740, -112.0740, -98.1234567890123456],
            'elevation_m': [331, 335, 331, 340, 350, 275]
        })
    
    def test_validates_coordinate_ranges(self, sample_coordinate_data):
        """Test that invalid coordinate ranges are detected."""
        result = validate_coordinate_data(sample_coordinate_data)
        
        assert isinstance(result, dict), "Function should return a dictionary"
        assert 'invalid_latitude' in result, "Should detect invalid latitude values"
        assert 'invalid_longitude' in result, "Should detect invalid longitude values"
        assert result['invalid_latitude'] > 0, "Should find invalid latitude values"
    
    def test_detects_missing_coordinates(self, sample_coordinate_data):
        """Test that missing coordinate values are detected."""
        result = validate_coordinate_data(sample_coordinate_data)
        
        assert 'missing_coordinates' in result, "Should detect missing coordinates"
        assert result['missing_coordinates'] > 0, "Should find missing coordinate values"
    
    def test_calculates_quality_score(self, sample_coordinate_data):
        """Test that quality score is calculated correctly."""
        result = validate_coordinate_data(sample_coordinate_data)
        
        assert 'quality_score' in result, "Should calculate quality score"
        assert 0 <= result['quality_score'] <= 100, "Quality score should be between 0-100"
    
    def test_handles_missing_columns(self):
        """Test function behavior with missing coordinate columns."""
        df_no_coords = pd.DataFrame({'station_id': ['STN_001'], 'elevation': [100]})
        result = validate_coordinate_data(df_no_coords)
        
        assert isinstance(result, dict), "Should return dict even with missing columns"
        assert 'recommendations' in result, "Should provide recommendations"


class TestMultiConditionFiltering:
    """Tests for the multi_condition_filtering function (Advanced Function 7)."""
    
    @pytest.fixture
    def sample_environmental_data(self):
        """Create complex environmental data for filtering tests."""
        return pd.DataFrame({
            'station_id': ['STN_001', 'STN_002', 'STN_003', 'STN_004', 'STN_005'],
            'temperature_c': [15.5, 25.2, 35.8, 18.1, 22.7],
            'humidity_percent': [45.2, 67.8, 89.1, 52.3, 61.4],
            'data_quality': ['good', 'fair', 'good', 'poor', 'good'],
            'elevation_m': [100, 250, 450, 180, 320]
        })
    ...
    def sample_filter_config(self):
        ...
        return {
            'temperature_range': {'min': 18.0, 'max': 30.0},
            'humidity_range': {'min': 50.0, 'max': 80.0},
            'quality_levels': ['good', 'fair'],
            'elevation_range': {'min': 150, 'max': 400}
        }
    
    def test_applies_multiple_filters(self, sample_environmental_data, sample_filter_config):
        """Test that multiple filter conditions are applied correctly."""
        result = multi_condition_filtering(sample_environmental_data, sample_filter_config)
        
        assert isinstance(result, pd.DataFrame), "Should return a DataFrame"
        assert len(result) <= len(sample_environmental_data), "Result should be same size or smaller"
    
    def test_handles_empty_config(self, sample_environmental_data):
        """Test function behavior with empty filter configuration."""
        empty_config = {}
        result = multi_condition_filtering(sample_environmental_data, empty_config)
        
        assert isinstance(result, pd.DataFrame), "Should return DataFrame with empty config"
    
    def test_complex_logical_operations(self, sample_environmental_data):
        """Test complex logical filter combinations."""
        complex_config = {
            'temperature_range': {'min': 20.0, 'max': 30.0},
            'quality_levels': ['good']
        }
        result = multi_condition_filtering(sample_environmental_data, complex_config)
        
        # All results should meet both temperature and quality criteria
        if not result.empty:
            assert all(result['temperature_c'] >= 20.0), "All results should meet min temperature"
            assert all(result['temperature_c'] <= 30.0), "All results should meet max temperature"
            assert all(result['data_quality'] == 'good'), "All results should have good quality"


class TestAnalyzeTemporalPatterns:
    """Tests for the analyze_temporal_patterns function (Advanced Function 8)."""
    
    @pytest.fixture
    def sample_temporal_data(self):
        """Create sample time series environmental data."""
        dates = pd.date_range('2023-01-01', periods=30, freq='D')
        return pd.DataFrame({
            'date': dates,
            'station_id': ['STN_001'] * 15 + ['STN_002'] * 15,
            'temperature': np.random.normal(20, 5, 30),
            'humidity': np.random.normal(60, 10, 30),
            'measurement_time': dates + pd.to_timedelta(np.random.randint(0, 24, 30), unit='h')
        })
    
    def test_analyzes_temporal_patterns(self, sample_temporal_data):
        """Test basic temporal pattern analysis functionality."""
        result = analyze_temporal_patterns(sample_temporal_data, 
                                        date_column='date', 
                                        value_column='temperature',
                                        groupby_column='station_id')
        
        assert isinstance(result, dict), "Should return a dictionary of results"
        assert 'temporal_stats' in result or 'trends' in result, "Should contain temporal analysis"
    
    def test_handles_datetime_conversion(self, sample_temporal_data):
        """Test that datetime conversion works properly."""
        # Convert dates to strings to test conversion
        string_data = sample_temporal_data.copy()
        string_data['date'] = string_data['date'].astype(str)
        
        result = analyze_temporal_patterns(string_data,
                                        date_column='date',
                                        value_column='temperature') 
        
        assert isinstance(result, dict), "Should handle string date conversion"
    
    def test_temporal_aggregation(self, sample_temporal_data):
        """Test temporal aggregation and grouping functionality."""
        result = analyze_temporal_patterns(sample_temporal_data,
                                        date_column='date',
                                        value_column='temperature',
                                        groupby_column='station_id')
        
        # Should analyze patterns for both stations
        assert isinstance(result, dict), "Should return analysis results"
    
    def test_handles_missing_date_column(self):
        """Test function behavior with missing date column."""
        df_no_date = pd.DataFrame({
            'station_id': ['STN_001', 'STN_002'],
            'temperature': [20.5, 22.1]
        })
        
        result = analyze_temporal_patterns(df_no_date, date_column='nonexistent_date')
        assert isinstance(result, dict), "Should return dict even with missing date column"
