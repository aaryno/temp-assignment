...
class TestValidateCoordinateData:
    ...
    def test_validate_coordinate_data_invalid_coordinates(self):
        ...
        data = {
            'station_id': ['A1', 'B2', 'C3'],
            'latitude': [40.7128, 91.0, -95.0],  # Invalid: >90, <-90
            'longitude': [-74.0060, -118.2437, 200.0]  # Invalid: >180
        }
        df = pd.DataFrame(data)
        
        result = validate_coordinate_data(df)
        
        # Should filter out invalid coordinates (medium implementation less sophisticated)
        assert len(result) >= 1, "Should keep at least one valid coordinate"
        assert len(result) < len(df), "Should filter out some invalid coordinates"
...
class TestMultiConditionFiltering:
    ...
    def test_multi_condition_filtering_numeric_ranges(self):
        ...
        # Should filter to temperature 20-30 (medium implementation may be less sophisticated)
        assert len(result) >= 2, "Should return some rows matching temperature range"
        assert len(result) <= 3, "Should filter out some rows"
...
class TestAnalyzeTemporalPatterns:
    ...
    def test_analyze_temporal_patterns_basic_functionality(self):
        ...
        data = {
            'date': dates,
            'temperature': [20, 22, 18, 25, 21] * (len(dates) // 5 + 1)[:len(dates)],
